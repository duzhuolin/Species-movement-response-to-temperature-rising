...
#parameters
landscape=500 #number of patches=landscape^2
coor.nwp=c(110,40) #coordinate(longitude,latitude) of the most northwestern point in the landscape
incre.lat=0.025 #increment of latitude between two adjacent patches on latitudinal direction
init.temp=30 #temperature at sea level on the latitude of landscape boundary near the equator
mean.temp=0.05 #mean of increment of temperature per year
var.temp=0.01 #standard deviation of temperature increment
...
var.stp=c(1:5) #options for step sizes that one species can make each time
...
for(m in 1:length(move.lkl)){
  for(rep in 1:replica){
   ...
   df=as.matrix(dem) #dem is a 6000x6000 matrix
   rowarray=round(seq(1,nrow(df),nrow(df)/landscape),0) #extract values from certains rows with same interval from df to build landscape
   land.elev=matrix(nrow=landscape,ncol=landscape)
   for(row in 1:landscape){
      land.elev[row,]=df[rowarray[row],round(seq(1,ncol(df),ncol(df)/landscape),0)] #extract values from certains columns with same interval from df to build landscape
   }
   ...
   land.temp=round(init.temp-(land.lat-land.lat[1,1])*0.8-land.elev*0.65/100,2)
   image(land.temp)
   ...
   for(s in 1:num.spe){
     ...
     lines(mvmt[seq(1,length(mvmt), 2)]/landscape, mvmt[seq(2,length(mvmt), 2)]/landscape, lwd=0.8)
