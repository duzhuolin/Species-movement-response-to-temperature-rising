...
elev.range=c(0,5000) #range of elevation within the landscape
...
move.lkl=c(0.3,0.8) #likelihood of species moving to coolest neighbor patch
...
for(m in 1:length(move.lkl)){
  for(rep in 1:replica){
   ...
   land.elev[xpeak, (ypeak+1):landscape] = round(rev(seq(elev.range[1], elev.range[2], (elev.range[2]-elev.range[1])/(landscape-ypeak-1)) + rnorm((landscape-ypeak),0,2)), 0)
   for(e in (xpeak-1):1){
     land.elev[e,] = land.elev[(e+1),] - round(rnorm(landscape, 0, 2), 0)
   }
   ...
   loc.spe=t(rbind(x,y))
   points(loc.spe[,1]/200, loc.spe[,2]/200, pch=19, cex=0.3)
   ...
   for(s in 1:num.spe){
     ...
     for(n in 1:num.years){
       ...
       if(move.temp[loc.x,loc.y]<=upper.spe[s]){
          ...
          if(loc.x==landscape | loc.y==landscape | loc.x==1 | loc.y==1){
             ...
             if(movecool==0){
               loc.x = sample(c(-1,0,1), 1) + loc.x
               loc.y = sample(c(-1,0,1), 1) + loc.y
               nxtpt=c(loc.x,loc.y)
             }
             if(movecool==1){
     ...
     lines(mvmt[seq(1,length(mvmt), 2)]/200, mvmt[seq(2,length(mvmt), 2)]/200, lwd=0.8)
